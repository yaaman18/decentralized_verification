...
3. 以後 DAO 参加者は DID によって DAO に入る
4. グループリーダーは DAO 内に何かしらのトピックを検証するグループを作る(function createGroup)
5. 他のメンバーは DID を提示してグループに参加する。（グループの履歴を記録するために外部の DB を利用するかは後ほど検討する）(function joinGroup)
6. グループメンバーがトピックを確認する
7. グループメンバーはグループ内でトピックに関する検証を行う（実験、論文の引用など）
8. テーマを検証する期間や手法は各グループ内で打ち合わせながら決めていく。notion や LINE などの外部ツールも自由に使用できる。
9. 検証が完了したらその結果について議論して評価をする為のレビュワーを公募する(function callForReviewers)
10. グループはトピックのレビュワー抽選に参加できる人数、実際のレビュワー人数、レビュワーが立候補できる期間、報酬体系や禁止ルールを設定する(function reviewSettings)
11. レビュワーが立候補する(function reviewerCandidacy)
12. レビュワーはトピックとその検証に対して興味を持った DAO のメンバーから無作為に選ばれる(function reviewerSelect)
13. レビュワーは不正防止の為にトークンプールコントラクトに一度トークンを預ける(function depositToken)
14. レビュワーとグループでの議論によってトピックと検証内容のレビューを行う。方法は自由だが透明性の担保の為に最終的には zoom などの議論の記録を動画として保存しておく。
15. 評価を結論づけたレビュワーから決定した内容（文章及び評価値）を投稿していく(function reviewResult)
16. トークンプールコントラクトがレビュワーへトークンを戻す(function refundDepositToken)
...
18. ERC721 に二次流通によるロイヤリティを付与するかも検討する。
    <br>
    <br>
    <br>
...
条件を変えてレビュワーを公募する --> 内容の稟議期間を設け後に検証側とその内容のレビュー側の打ち合わせをzoomなどで記録する
指定した条件によって選出する --> 内容の稟議期間を設け後に検証側とその内容のレビュー側の打ち合わせをzoomなどで記録する
